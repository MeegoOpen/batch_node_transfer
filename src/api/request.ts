...
import { requestHost, loginUrl } from "../constants/index";
...
const toastCallBack = (e: Error) => {
  ...
  if (!Object.prototype.hasOwnProperty.call(toastCache, e.message)) {
    ...
    sdk.toast.error(e.message).then(() => {
...
request.interceptors.response.use(
  async (response) => {
    ...
    let logId = response.headers?.["x-tt-logid"];
    if (res.code !== 0) {
      logId = response.headers?.["x-tt-logid"];
      // token 失效
      if (res.code === 1000052203 || res.code === 1000052013) {
        await LoginManager.getInstance().removeToken();
      }
      if (
        ![1000052203, 1000052020].includes(res.code) &&
        !response?.config?.url?.includes("node_operation")
      ) {
        // 特化: 飞书审批定义接口不弹 toast
        toastCallBack(new Error(res.msg));
      }
      if (res.code === 1000051887) {
        toastCallBack(new Error("服务器繁忙，请稍后再试"));
      }
    }
    return { ...res, logId, code: response.data.err_code };
  },
  (error) => {
    // 对响应错误做些什么
    // toastCallBack(error);
